<template>
  <v-dialog v-model="dialog" persistent max-width="500px">
    <template v-slot:activator="{ on }">
      <v-btn color="primary" dark class="mb-2" v-on="on">Draft a new loan</v-btn>
    </template>
    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="form">
          <v-container>
            <BorrowerSelect :rules="rules.borrower" @change="changeBorrower" />
            <v-text-field
              v-model="loan.amount"
              label="Amount"
              type="number"
...
export default {
  ...
  computed: {
    rules() {
      return {
        borrower: [v => !!v || "Borrower is required"],
        amount: [
        ...
        installmentCount: [
          ...
          v =>
            ...
            "Installment count should be a whole number",
